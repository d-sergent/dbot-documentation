...
Ce document consolidé réunit les spécifications validées, l'étude de montage mécanique, la préparation URDF et le rapport d'audit final pour le sous-assemblage **Tête et Cou (2-DOF : Yaw & Pitch)** du D-Bot.

---

### 📄 SOURCE CONSOLIDÉE : FINAL_CONSOLIDE_Tete_et_Cou.md
...
## 📑 Sommaire
  ...
  - [📄 SOURCE CONSOLIDÉE : FINAL_CONSOLIDE_Tete_et_Cou.md](#source-consolidée-finalconsolideteteetcoumd)
...
- [**8. Validation CAO Haute Précision (Fusion 360)**](#8-validation-cao-haute-précision-fusion-360)
  ...
  - [📄 SOURCE D'ÉTUDE : STUDY_Montage_Cou.md](#source-détude-studymontagecoumd)
...
- [**12. Système de Verrouillage Statique du Tilt (Parking Brake)**](#12-système-de-verrouillage-statique-du-tilt-parking-brake)
  ...
  - [📄 SOURCE COMPLÉMENTAIRE : FINAL_URDF_Cou.md](#source-complémentaire-finalurdfcoumd)
...
En tant qu'Ingénieur Senior en Revue de Conception, j'ai examiné les documents fournis pour le module Tête et Cou du projet D-Bot. Ce rapport présente mes observations critiques, les dépendances identifiées, les manques et incertitudes, ainsi que des propositions d'amélioration pour la configuration **2-DOF (Yaw & Pitch)**.

## 0. Décision d'Architecture Retenue

Le module Tête et Cou du D-Bot est conçu pour offrir **2 Degrés de Liberté (DOF) : Yaw (Pan) et Pitch (Tilt)**, chacun motorisé par un actionneur RobStride RS-05 en entraînement direct (ratio 1:1). L'architecture privilégie la robustesse et la protection des moteurs en déportant les charges statiques et dynamiques sur des roulements externes :
*   **Yaw (Pan) :** Joint révolute à axe vertical (Z). Le moteur vertical est fixé sur le torse, son rotor pointant vers le haut. Il est supporté par un montage pivot indépendant (**Solution 4** avec roulement 6804-2RS et Tube Hub) reprenant l'intégralité du poids de la tête (2 kg).
*   **Pitch (Tilt) :** Joint révolute à axe horizontal (Y). Il utilise un montage en chape double palier (**Yoke Mount** avec deux roulements 6804-2RS de part et d'autre) et comporte deux solénoïdes de type **LEX-SOLEN-04** pour le freinage statique (Parking Brake).

| Choix de Conception Final | Justification Technique |
| :------------------------- | :----------------------- |
| **Yaw (Pan)** | `neck_yaw_joint` | `0 0 1` (Z) | [-1.57, 1.57] | ±90° | 5.5 N.m | 3.14 rad/s |
| **Pitch (Tilt)** | `neck_pitch_joint` | `0 1 0` (Y) | [-0.79, 0.79] | ±45° | 5.5 N.m | 3.14 rad/s |
| **Support Yaw** | Roulement 6804-2RS | Hub réduit (Solution 4) | Protège les roulements internes du RS-05 des charges axiales (poids de la tête) et moments de basculement. |
| **Support Pitch** | 2x Roulement 6804-2RS | Montage "Yoke Mount" (Chape) | Assure la coaxialité parfaite et déporte les charges radiales et le moment fléchissant de la tête. |
| **Verrouillage Pitch** | 2x Solénoïdes LEX-SOLEN-04 | Parking Brake (Power to Unlock) | Immobilise la tête en position de repos ou d'urgence (Normally Locked), économise l'énergie des moteurs. |
...
1.  **Conversion Vitesse Max (rpm vers rad/s)**
    ...
    *   **Valeur documentée:** 3.14 rad/s.
    *   **Observation:** ✅ **RÉSOLU** - L'incohérence a été résolue en adoptant **3,14 rad/s (180°/s)** comme limite opérationnelle de vitesse maximale URDF pour tous les joints du cou (Yaw/Pitch). La capacité matérielle maximale du moteur reste de 12,57 rad/s (120 rpm) à vide, mais le bridage opérationnel à 3,14 rad/s protège la mécanique des contraintes d'inertie de la tête de 2 kg et prévient le flou sur la caméra OAK-D Pro FF.
    *   **Recommandation:** Maintenir cette limite logicielle dans les configurations du planificateur de trajectoires ROS 2 (avec une régulation logicielle usuelle à 1,57 rad/s pour un rendu naturel).
...
2.  **Conversion Limites Angulaires (rad vers °)**
    ...
    *   **Pitch:** `0.79 rad * (180/π) ≈ 45.25°`. Documenté: ±45°. **Cohérent.**

3.  **Marge Radiale Roulement 6804-2RS (Yaw)**
    ...
    *   **Observation:** Le calcul est correct et la marge confortable de 4.75 mm est **validée**. Elle permet un carter avec une paroi d'au moins 3 mm retenant la bague extérieure.

4.  **Couple d'Accélération Requis pour le Yaw**
    *   **Source:** `STUDY_Montage_Cou` (§6.1).
    *   **Équation:** `T_acc = I_yaw × α`
    *   **Données:** m = 2 kg, r = 0.1 m, I_yaw ≈ 0.01 kg.m², t_acc = 0.1 s, ω = 3.14 rad/s.
    *   **Calcul:** Accélération angulaire `α = 3.14 / 0.1 = 31.4 rad/s²`. Couple d'accélération requis `T_acc = 0.01 × 31.4 = 0.314 N.m` (ou ~0.6 N.m en incluant marge dynamique et friction).
    *   **Valeur documentée:** ~0.6 N.m.
    *   **Observation:** Le calcul est physique et cohérent. Le couple gravitationnel statique direct étant nul sur cet axe vertical, le moteur RS-05 travaille avec de larges marges de sécurité.

5.  **Marge de Couple du RS-05 (Yaw)**
    *   **Source:** `STUDY_Montage_Cou` (§6.2).
    *   **Données:** Couple dynamique requis = ~0.6 N·m. Couple nominal RS-05 = 1.6 N·m. Couple pic RS-05 = 5.5 N·m.
    *   **Calcul:** Marge nominale = `1.6 / 0.6 ≈ 2.67`. Marge pic = `5.5 / 0.6 ≈ 9.17`.
    *   **Valeurs documentées:** Marge nominale ×2.6, Marge pic ×9.1.
    *   **Observation:** Les calculs sont corrects et les marges d'accélération sont **validées**.

6.  **Coût Total Matériel Estimé (BOM Locale)**
    *   **Source:** `FINAL_CONSOLIDE` (§3 Nomenclature (BOM Locale)).
    *   **Calcul des éléments listés:**
        *   3x Roulement 6804-2RS (1 Yaw + 2 Pitch) : 3 * 4 € = 12 €
        *   2x Circlip E20: 2 * 0.30 € = 0.60 €
        *   2x Solénoïde LEX-SOLEN-04: 2 * 3 € = 6 €
        *   1x Visserie M3 / M4: 0.50 €
        *   **Sous-total:** `12 + 0.60 + 6 + 0.50 = 19.10 €`
    *   **Valeur documentée:** ≈ 19 €.
    *   **Observation:** ✅ **RÉSOLU** - L'incohérence a été résolue en ajoutant le second roulement de support de Pitch omis (soit 3 roulements au total dans la BOM cou) et en harmonisant le coût total BOM locale à **19 €** (au lieu de 15 € précédemment), ce qui correspond exactement au total cumulé des composants listés révisé.

7.  **Charges Dynamiques Tête (Pitch)**
...
8.  **Moment de Flexion et Force de Réaction (Pitch)**
    *   **Source:** `STUDY_Montage_Cou` (§10.3 Analyse des Charges).
    *   **Données:** Moment de flexion = ~3 N·m. Bras de levier = ~100 mm (0.1 m). Force de réaction par roulement = ~15 N.
    *   **Calcul de la force de réaction répartie sur deux roulements (chape double) :** `F_par_roulement = M / (2 × L) = 3 N·m / (2 × 0.1 m) = 15 N`.
    *   **Valeur documentée:** ~15 N.
    *   **Observation:** ✅ **CORRIGÉ** - L'incohérence de calcul a été corrigée. La valeur de la force de réaction par roulement est désormais de **15 N** (au lieu de 18 N), ce qui est parfaitement exact d'un point de vue de la statique des poutres en chape double.

9.  **Marge Capacité Radiale Roulement 6804-2RS (Pitch)**
    *   **Source:** `STUDY_Montage_Cou` (§10.3).
    *   **Données:** Capacité radiale 6804-2RS = ~3.1 kN. Efforts normaux (force par roulement) = ~15 N.
    *   **Calcul:** Marge = `3100 N / 15 N ≈ 206.6`.
    *   **Valeur documentée:** Marge × 200.
    *   **Observation:** Le calcul est cohérent et la marge de sécurité (supérieure à ×200) est **validée**.

10. **Bilan Électrique 12V (Solénoïdes + Hub USB)**
    ...
    *   **Calcul:** Courant total = `1.2 A + 2.5 A = 3.7 A`. Marge = `(5 A - 3.7 A) / 5 A = 26%`.
...
Le module Tête et Cou présente des interdépendances critiques avec d'autres sous-systèmes du D-Bot :
*   **[Masse de la tête (≈ 2 kg)]** → **[Torse / Base du robot]** → **[Stabilité globale, Centre de Gravité (CdG)]** : La masse de la tête (allégée de ~350g par le passage au 2-DOF) est un facteur majeur pour le calcul du CdG global du robot, impactant la stabilité en marche et les couples requis aux hanches et aux chevilles.
*   **[Couple requis pour Yaw/Pitch (5.5 N·m pic)]** → **[Système d'alimentation électrique (PDB, batteries)]** → **[Consommation de courant, dimensionnement du bus 48V]** : Les besoins en couple des moteurs RS-05 déterminent la puissance électrique nécessaire, influençant la capacité des batteries et le dimensionnement des régulateurs de puissance.
...
*   **[Vitesse max des joints (3.14 rad/s)]** → **[Contrôle moteur / Planification de trajectoire (ROS 2)]** → **[Dynamique des mouvements, temps de réponse, fluidité]** : La vitesse maximale de 3,14 rad/s (180°/s) est le paramètre de référence pour la planification de trajectoire dans MoveIt et la génération des profils de mouvement fluides.
*   **[Rigidité du bracket en U (Pitch) / Précision de coaxialité des roulements]** → **[Système de Vision (Capteurs OAK-D Pro FF)]** → **[Précision de la perception, stabilité de l'image]** : Toute déformation du bracket en U ou désalignement des roulements peut introduire du jeu ou des vibrations, dégradant la précision du SLAM et la qualité des données de vision de la caméra Fixed Focus.
...
Cette section met en évidence les points nécessitant une clarification, une justification ou une validation supplémentaire dans le cycle de développement restant :

1.  **Incohérence Vitesse Max Moteur/Joints**
    *   **Risque:** ✅ **RÉSOLU** - L'incohérence a été résolue en spécifiant **3,14 rad/s (180°/s)** comme limite logicielle et opérationnelle des joints du cou dans l'URDF et les spécifications consolidées.

2.  **Incohérence Coût BOM**
    *   **Risque:** ✅ **RÉSOLU** - L'incohérence a été résolue en passant la quantité de roulements de Pitch à 2 (soit 3 au total pour le cou) et en harmonisant le coût total BOM locale à **19 €** (au lieu de 15 € précédemment), ce qui correspond exactement au total cumulé révisé des composants modélisés en CAO.

3.  **Incohérence Calcul Force de Réaction Pitch**
    *   **Risque:** ✅ **RÉSOLU** - L'incohérence de calcul a été corrigée. La valeur de la force de réaction par roulement est désormais de **15 N** (au lieu de 18 N), ce qui est parfaitement exact.

4.  **Incohérence Tension d'Alimentation Moteurs (24V vs 48V)**
    *   **Risque:** ✅ **RÉSOLU** - L'incohérence a été résolue en corrigeant la tension d'alimentation nominale des moteurs RS-05 de 24V à **48V DC nominal** (plage de 15-60V supportée en interne). Les RS-05 du D-Bot sont alimentés **directement** depuis le bus d'alimentation central de 48V (identique aux moteurs RS-02, RS-03 et RS-04 du reste du robot), évitant le besoin inutile d'un lourd convertisseur abaisseur 48V→24V dans le torse et libérant 100% de la dynamique du moteur (le fonctionnement sous 24V bridant le couple et la vitesse de ~50%). La tension de 24V est uniquement réservée pour la protection matérielle lors des phases de tests individuels sur banc de laboratoire (alimentation Wanptek).

5.  **Justification du Facteur Dynamique 1.5g**
    *   **Risque:** 🟡 **À SURVEILLER** - L'hypothèse de 1.5g pour les charges dynamiques de la tête n'est pas pleinement justifiée par des mesures. Un choc violent (chute, arrêt brusque en marche) pourrait dépasser cette valeur.
    *   **Action Recommandée:** Valider ce facteur par des tests expérimentaux (accéléromètre dans la tête) lors des premières marches du robot.

6.  **Fournisseurs des Pièces Usinées (Bracket U-Pan, Tube Hub, Carter fixe)**
    *   **Risque:** 🟠 **IMPORTANT** - Retard dans la fabrication et l'approvisionnement des pièces critiques (les champs sont toujours marqués `[À COMPLÉTER]` dans la BOM).
    *   **Action Recommandée:** Finaliser la sélection des fournisseurs (usinage interne ou sous-traitance type Protolabs) et obtenir des devis fermes.

7.  **Gestion Thermique des Moteurs RS-05**
    *   **Risque:** 🟠 **IMPORTANT** - Surchauffe du moteur Pitch en charge continue en cas de maintien prolongé de postures inclinées sous couple gravitationnel (0.69 N.m).
    *   **Action Recommandée:** Réaliser des tests de charge continue (e.g., maintien d'inclinaison à 45° pendant 30 min) pour mesurer la température et valider l'absence de surchauffe.

8.  **Documentation du Câblage Électrique**
    *   **Risque:** 🟡 **À SURVEILLER** - Difficultés de montage, de maintenance et de dépannage sans schéma électrique clair pour l'alimentation 48V des 2 moteurs et la commande 12V des solénoïdes.
    *   **Action Recommandée:** Créer un schéma électrique détaillé.

9.  **Validation du Modèle Dynamique Gazebo (Inerties)**
    *   **Risque:** 🟡 **À SURVEILLER** - Des inerties incorrectes dans le modèle URDF (les tenseurs d'inertie n'y étant pas détaillés) peuvent fausser les gains de contrôle dans les simulations.
    *   **Action Recommandée:** Extraire les tenseurs d'inertie réels du modèle 3D Fusion 360 pour le Yaw et le Pitch, et les injecter dans le fichier URDF.

10. **Rigidité du Bracket Pitch en U**
    *   **Risque:** 🟡 **À SURVEILLER** - Une déformation ou flexion du bracket en U pourrait nuire à la coaxialité des paliers de Pitch et induire du frottement permanent sur les roulements 6804-2RS.
    *   **Action Recommandée:** Réaliser une analyse par éléments finis (FEA) sur le bracket en U.

11. **Validation du Temps de Réponse du Parking Brake**
    *   **Risque:** 🟢 **SUGGESTION** - Le temps de réponse de 50ms est une estimation théorique.
    *   **Action Recommandée:** Mesurer physiquement le temps d'engagement/désengagement des solénoïdes LEX-SOLEN-04 sous 12V pour optimiser la synchronisation ROS 2.

---

## 4. Propositions d'Amélioration

Voici trois propositions concrètes pour améliorer la conception actuelle dans les limites de la version V1.x :

1.  **Standardisation des Pièces Usinées (Hubs et Carters)**
    *   **Description:** Conserver des plans strictement identiques et interchangeables pour les "Tube Hub" et "Carter fixe" du Yaw et du Pitch.
    *   **Bénéfice:** Réduction des coûts d'usinage (économies d'échelle), simplification des stocks et élimination des risques d'inversion lors de l'assemblage.
    *   **Action:** Valider l'interchangeabilité des hubs et carters Yaw/Pitch sur les fichiers CAD.

2.  **Intégration de Capteurs de Température Moteur**
    *   **Description:** Exploiter des sondes ou thermistances internes pour surveiller la température en continu, notamment sur le moteur Pitch qui lutte contre la gravité.
    *   **Bénéfice:** Protection thermique proactive (coupure de sécurité ou retour en position neutre assistée par le Parking Brake en cas de surchauffe).
    *   **Action:** Configurer les remontées de température via le bus CAN du RS-05 dans le driver ROS 2.

3.  **Analyse par Éléments Finis (FEA) du Bracket Pitch en U (Yaw Bracket)**
    *   **Description:** Réaliser une simulation FEA du bracket en chape pour valider sa rigidité mécanique et optimiser son épaisseur (gain de masse).
    *   **Bénéfice:** Garantie de coaxialité parfaite sous charge dynamique, améliorant la durée de vie des roulements et la précision des capteurs de vision.

---

## 5. Synthèse du Niveau de Maturité

★★★★☆ — La conception mécanique 2-DOF (Yaw & Pitch) est désormais **très robuste, cohérente et géométriquement validée** par prototype. Les calculs physiques de couple, de marge de roulement, de charges dynamiques et de bilan électrique sont parfaitement alignés et exempts de contradictions. La maturité globale est excellente, les seuls manques restants concernant les validations expérimentales d'usage (thermique, rigidité du bracket en chape et finalisation des fournisseurs de pièces usinées).
