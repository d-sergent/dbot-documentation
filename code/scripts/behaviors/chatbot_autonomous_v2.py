"""
chatbot_autonomous_v2.py — Version PRODUCTION (Headless).
===========================================================================
- Utilise audio_io_autonomous.py (ALSA Direct exclusif)
- Aucun dépendance sur PulseAudio
- Optimisé pour le fonctionnement sans écran (Jetson 8GB)
"""
...
from dbot.audio.audio_io_autonomous import AudioIOAutonomous
...
def on_doa_update(angle: int):
    print(f"🧭 [DOA] Source sonore détectée à {angle}°.")

def main():
    print("🤖 === D-Bot : Chatbot AUTONOME (Production — ALSA Direct) === 🤖\n")

    # --- INITIALISATION AUDIO (ALSA Direct) ---
    audio = AudioIOAutonomous(doa_callback=on_doa_update)
    ...
    try:
        ...
        print(f"✅ [Système] D-Bot prêt en mode PRODUCTION.")
    except Exception as e:
        print(f"\n❌ Erreur initialisation IA : {e}")
        sys.exit(1)

    # Fonction de parole spécifique ALSA Direct
    def speak_alsa(text):
        wav_path = "/tmp/dbot_response.wav"
        tts.generate_wav(text, wav_path)
        audio.play_audio(wav_path)
        if os.path.exists(wav_path): os.remove(wav_path)

    speak_alsa("Système autonome initialisé. Je vous écoute.")

    print("\n👀 À l'écoute (VAD matériel)... (Ctrl+C pour quitter)\n")

    try:
        while True:
            wav_path = "/tmp/dbot_input_autonomous.wav"
